...
import librec.intf.CopyOfRecommender.Measure;
import librec.main.BPRRanking;
import librec.main.BiasedMFRanking;
...
public abstract class Recommender implements Runnable {
	...
	public enum Measure {
		...
		D5, D10, Pre1, Pre2, Pre5, Pre10, Rec5, Rec10, MAP, MRR, NDCG, AUC,
		/* execution time */
		TrainTime, TestTime,
		/* loss value */
		Loss
	...
	public static String getEvalInfo(Map<Measure, Double> measures) {
		...
		if (isRankingPred) {
			if (isDiverseUsed)
				...
				evalInfo = String.format("%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f", 
						measures.get(Measure.Pre1), measures.get(Measure.Pre2), measures.get(Measure.Pre5),
	...
	protected Map<Measure, Double> evalRatings() throws Exception {
		...
		for (MatrixEntry me : testMatrix) {
			...
			if (isResultsOut) {
				...
				String itemId = rateDao.getItemId(j);
				
				if(BiasedMFRanking.ratingMap.containsKey(uid))
				{
					BiasedMFRanking.ratingMap.get(uid).put(itemId, pred);
				}
				else
				{
					TreeMap<String,Double> m = new TreeMap<String,Double>();
					m.put(itemId, pred);
					BiasedMFRanking.ratingMap.put(uid, m);
				}
	...
	protected Map<Measure, Double> evalRankings() throws Exception {
		...
		System.out.println("Recommender 692 capacity:"+capacity);
		...
		for (int u = 0, um = testMatrix.numRows(); u < um; u++) {
			...
			//System.out.println("Recommender 787 itemScore.size:"+itemScores.size());
			List<Map.Entry<Integer, Double>> recomd = (numRecs <= 0 || itemScores.size() <= numRecs) ? itemScores
					: itemScores.subList(0, numRecs);
			//System.out.println("Recommender 790 recomd.size:"+recomd.size());
			
			List<Integer> rankedItems = new ArrayList<>();
			StringBuilder sb = new StringBuilder();
			int count = 0;
			
			BPRRanking.recommendMap.put(rateDao.getUserId(u),new ArrayList<String>());
			...
			for (Map.Entry<Integer, Double> kv : recomd) {
				...
				if (isResultsOut && count < numTopNRanks) {
					...
					*/
					...
					{	
						sb.append("(").append(rateDao.getItemId(item));
						//System.out.println(+" "+rateDao.getItemId(item)+" "+kv.getValue().floatValue());
						String itemId = rateDao.getItemId(item);
						BPRRanking.recommendMap.get(rateDao.getUserId(u)).add(itemId);
			...
			List<Integer> cutoffs = Arrays.asList(1, 2, 5, 10);
