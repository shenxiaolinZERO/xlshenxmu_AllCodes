 ...
 */
@Configuration
public abstract class CopyOfRecommender implements Runnable {
	 ...
	 *            test matrix
	 */
	public CopyOfRecommender(SparseMatrix trainMatrix, SparseMatrix testMatrix, int fold) {
	...
	private void printAlgoConfig() {
		...
		Class<? extends CopyOfRecommender> cl = this.getClass();
		// basic annotation
	...
	protected Map<Measure, Double> evalRankings() throws Exception {
		...
		for (int u = 0, um = testMatrix.numRows(); u < um; u++) {
			...
			BiasedMFRanking.recommendMap.put(rateDao.getUserId(u),new ArrayList<String>());
				...
				if (isResultsOut && count < numTopNRanks) {
					...
					*/
					...
					{	
						...
						BPRRanking.recommendMap.get(rateDao.getUserId(u)).add(rateDao.getItemId(item));
						//	sb.append("*"); // indicating correct recommendation
