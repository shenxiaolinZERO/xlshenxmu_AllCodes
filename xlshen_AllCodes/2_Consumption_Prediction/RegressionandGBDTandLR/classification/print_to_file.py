...
# 输入： 多个list组成的list，每个list输出为一行，如[[1,2,3,4],[5,6,7,8]]
# 输出： 每个list输出为一行，格式为： 1,2,3,4    5,6,7,8
# 如果cur_file_path指定的文件已存在，则会重新生成并覆盖
def print_lists_to_file(lists, file_path):
...
def print_data_lists_to_file(data_lists, write_path):
    ...
    for cur_data_list in data_lists:
        x = cur_data_list[0]
        x_list1 = cur_data_list[1]
        x_list2 = cur_data_list[2]
