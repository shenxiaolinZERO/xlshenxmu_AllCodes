...
class Recommendation11:
    ...
    @staticmethod
    def generate(U, V, theta, aspects_num, session_item_data, dic, item_session_times_dic,res_dir):
...
def calc_item_prob2(V, theta, aspects_num, dic, session_aspect_dic, item_session_times_dic):
   #参数dic：表示当前数据中某个session与"其购买+点击商品的list"的map
   #参数item_session_times_dic：表示每个商品在各个session的出现次数
    session_item_prob_dic = dict()
