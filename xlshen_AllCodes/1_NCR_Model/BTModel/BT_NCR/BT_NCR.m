...
for i = 1:cvObj.NumTestSets
    ...
    out(i,:) = BT_NCR_prediction(session,testIdx,U,V,theta,par);
...
for i = 1:par.maxIter
    tic;
    gammaOut = cellfun(@(x) gammaFunc(x,U,V,theta,par),cellD,'UniformOutput',false); 
    ...
    temp = thetaFunc(matD(:,2:3),gammaOut,V,theta,par); 
    theta = thetaTop/temp;
    fprintf('update theta completed\n')
    lh = cellfun(@(x) lhFunc(x,U,V,theta,par),cellD); 
    lh = sum(log(lh));
    t=toc;
    ...
    out = BT_NCR_prediction(session,testIdx,U,V,theta,par); 
    fprintf('BT_NCR : iter [%d/%d] completed, theta = %f, time = %f, loss = %f, delta_loss = %f\n',i,par.maxIter,theta,t,lh,deltaLoss);
