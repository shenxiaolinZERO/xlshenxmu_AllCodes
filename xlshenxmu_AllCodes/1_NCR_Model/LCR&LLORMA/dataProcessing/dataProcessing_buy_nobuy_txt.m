...
%rawData = dlmread('user_log_format1.csv',',',1,0);
%tabulate(data(:,end));
...
for i = 1:length(idx)
    isBuy = find(rawData(p{idx(i)},5)==4,1);
    if ~isempty(isBuy)
...
while 1
    ...
    buyLen = arrayfun(@(x) length(find(rawData(temp(p{x}),5)==4)),1:length(sessSet));
...
t=toc;
fprintf('step5 - renumbering and generating data completed, data amount: %d, time: %f\n',length(rawData),t);
rawData = rawData(:,[1,2,5]); 
fid=fopen('Tmall_hybrid.txt','w');   %新建一个txt文件存放结果
fprintf(fid,'%d,%d,%d\n',rawData');
fclose(fid);
