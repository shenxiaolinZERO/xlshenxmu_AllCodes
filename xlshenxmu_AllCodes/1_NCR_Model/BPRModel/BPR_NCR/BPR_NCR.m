function [U,V,Theta] = BPR_NCR(seed,session,varargin)
rng(seed)
%% Parse parameters
params = inputParser;
...
%     toc
    filename = sprintf('BPR_NCR_FilmTrust_fold_%i.mat',i);
    save (filename,'testIdx','U','V','Theta','par','-mat');
    out(i,:) = BPR_NCR_prediction(session,testIdx,U,V,Theta,par);
    fprintf('BPR:fold [%d/%d] completed\n',i,cvObj.NumTestSets);
end
%---------- 保存结果---start-------
resname = sprintf('BPR_NCR_FilmTrust_5fold_result.mat');
meanValue=mean(out);
allOut = out;
...
for i = 1:par.maxIter
    ...
    out = BPR_NCR_prediction(session,testIdx,U,V,Theta,par);
    if par.adaptive && i > 2
    ...
    fprintf('BPR_NCR : iter [%d/%d] completed,deltayU = %f,deltaV = %f\n',i,par.maxIter,abs(deltaU),abs(deltaV));
